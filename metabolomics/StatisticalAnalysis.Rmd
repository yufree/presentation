...
???
In mass spectrumetry dataset, the EIC is not that simple for full scan. Due to the accuracy of instrument, the detected mass to charge ratio would have some shift and EIC would fail if different scan get the intensity from different mass to charge ratio. In the matchedfilter algorithm, they solve this issue by bin the data in m/z dimension. The adjacent chromatographic slices could be combined to find a clean signal fitting fixed second-derivative Gaussian with full width at half-maximum
...
```{r peakfilling, fig.show='hold', echo=FALSE}
...
- Statistics descripe certain property among the samples
        
- Statistics could be designed for certain purpose

- Statistics extract single and remove noise
...
- One comparision test, false positive 

$$ 1- (1 - 0.05) = 0.05$$

- Two comparision tests, false positive 

$$1 - (1 - 0.05)^2 = 0.0975$$

- Ten comparision tests, false positive
...
- $\alpha$ means the cutoff of p value, i means the rank of certain test and m means the numbers of comparision

- Adjested p value for FDR control
...
- Diagnoised by other statistics( $R^2$, $ROC$)
...
- If the parameter show no signifituer differences with 0, the group information might show limited contribution for this peak intensity
...
- Data points on time series has autocorrelation

- Regression analysis is not suitable for time series

- Survial analysis might also be used in certain context
...
- Use bio-infomation to explain the mechanism
...
???
This is actually the same in analytical chemistry. A higher sensitivity model could avoid false negative. A higher specificity model could avoid false positive. The accuracy would not distinguish such things.
...
- use priciple components to show the major variances
- each priciple component is a liner combination of peaks with loadings

### Samples could be visulized in 2D/3D mode in score plot

- When the first 2or3 priciple components could show 80% variances

- If the samples are clustered, they might be similar on the major priciple components


???
PCA is an Exploratory Data Analysis(EDA) method, not statistical inference with p value. Conclusion should be valided by extra statistical methods or simulation
...
- For PCA, both new variables and classes are orthognal
- For PLSDA(Wold), only new classes are orthognal. 
- For PLS(Martens), only new variables are orthognal. 

### Sparse PLS-DA make a L1 penal on the variable selection

- remove the influnces from unrelated variables
...
- Use varibales' distances among samples to show inner relationship
...
- At each level, different variables play roles in seperation of samples

- Each branchs of the tree belong to certain groups
...
???
peaks could be used in dfferent levels for seperation
...
- Use multiple trees to vote for the seperation

- Variable importance could be computed by the influnces on the seperation
...
- New trees are shrinked to fit the residual errors

$$\hat f(x) = \sum_{b=1}^B \lambda \hat f^b(x)$$
- When depth is 1, boosting is identical with addtive model

---

## Support vector machine

- p-dimensional vector could be seperate by (p-1)-dimensional hyperplane

- Find the hyperlane to make largest separation between gourps

- Kernel function is used to map the higher-dimensional into a lower-dimensinal space
...
Hidden layer use different kernal functions
