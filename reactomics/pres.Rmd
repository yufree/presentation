...
---
## Why PMD? PMDs are quantized shells

```{r echo=FALSE, out.width= '60%'}
knitr::include_graphics('https://yufree.github.io/reactomics/assets/pmd-orbitals.png')
```

*Like electron orbitals, paired mass distances are not continuous — they are quantized. Each shell encodes one biochemical transformation.*

???
- Real datasets are dominated by a countable set of discrete PMDs, not a continuum
- This orbital picture is more than a metaphor: the discreteness follows from structural constraints rooted in physics and evolution
- Two questions follow: where do the shells come from, and why can mass spectrometry read them?

---

## Where the quantization comes from

- Quantum mechanics: the constraint is the Schrödinger equation — every bound electron occupies one of countable eigenstates, shared by every atom

- Metabolism: the constraint is the **early cofactor budget** — almost every reaction transfers a group from a small pre-LUCA cofactor set
...
- The PMDs dominating real data are exactly the ones this cofactor–enzyme infrastructure delivers cheaply

???
- Every cofactor delivers a fixed mass increment — that increment is the PMD
- Chemically interesting PMDs requiring unavailable cofactors or higher activation barriers never show up at high frequency
- The discreteness is the shadow of which transformations early life could afford

---

## Why mass spectrometry can read the shells

- PMDs are not integer masses — +CH₂ is 14.0157 Da while +N is 14.0031 Da; those fractional offsets come from **nuclear binding energy**, not organic chemistry

- The same physical quantization that gives atoms discrete energy levels gives molecular reactions discrete mass signatures

> Quantization at the electronic level made the chemistry possible; quantization at the nuclear level made it readable.

- HRMS (Orbitrap, FT-ICR) is essentially a nuclear-binding-energy detector dressed up as a chemistry instrument

???
- Nucleons binding into a nucleus release energy; through E = mc² it registers as a mass deficit
- Carbon-12 is 12 by convention; H-1 is 1.00783 Da, O-16 is 15.9949 Da, N-14 is 14.0031 Da
- Without this layer the PMD shells would collapse onto the integer-mass axis and reactomics would be impossible

---

## The evolutionary freeze

- Once a cofactor becomes central to metabolism, it cannot be removed — every downstream pathway reads it, the same logic that froze the genetic code

- The high-frequency PMD spectrum is **conserved across all domains of life**

- Bacteria, plants and humans share roughly the same dominant PMDs because they share the same cofactor set

- What differs between organisms is the *intensity* of each shell — which reactions are up- or down-regulated

> Universality through a common underlying constraint: every atom obeys the same Schrödinger equation; every organism inherited the same frozen cofactor set.

???
- Chemistry has its own genuine quantization principle: cofactor inheritance plus evolutionary lock-in
- No need to borrow the prestige of physics — the discreteness of the metabolic reaction set is a phenomenon in its own right, with its own physical mechanism and its own conservation law
...
- **Static PMD ⇒ upstream/downstream control** — the enzyme runs stably; what changes is substrate supply or product removal. Quantify by intensity **sum**

- **Dynamic PMD ⇒ enzyme-level control** — enzyme activity is the variable; substrate supply is roughly constant. Quantify by **ratio** (stable peak as internal reference)

- Conceptually parallel to metabolic control analysis (MCA), but read directly from untargeted LC-MS data

    - No kinetic measurements
    - No isotopically labelled tracers
    - No compound identification

???
- Static pairs mean the enzyme is not the rate-limiting step — total throughput (sum) tracks upstream supply and downstream demand
- Dynamic pairs mean the enzyme itself is regulated — the ratio isolates catalytic activity from sample-level abundance drift

---

## Reaction-level quantification: `getreact()`

- The unit of analysis is the **reaction, not the molecule**

- One PMD (e.g. 15.9949 Da, +O) can appear thousands of times — every occurrence is one observation of that reaction happening

- `getreact()` in the `pmd` package returns a **reaction × sample matrix** for statistical comparison

```{r eval=FALSE}
getreact(pmd, method = "static")   # intensity sum: upstream/downstream regulation
getreact(pmd, method = "dynamic")  # stable/variable ratio: enzyme-level regulation
```

> Building networks alone does not free the analysis from identification — reaction-level quantification is what separates reactomics from yet another network method

???
- Reactions are the analytes: quantify oxidation, methylation or glycosylation activity directly, with no compound list
- This is the most under-appreciated feature of reactomics and the reason it deserves the omics suffix
...
## Step 4: Quantify reactions

- `getreact()` returns a reaction × sample matrix

- Static mode (intensity sum) or dynamic mode (stable/variable ratio)
...
```{r echo=FALSE}
knitr::include_graphics('https://yufree.github.io/presentation/figure/bags.png')
```
...
knitr::include_graphics('https://yufree.github.io/presentation/figure/rpppmdda2.png')
```
...
knitr::include_graphics('https://yufree.github.io/presentation/figure/netdemo.png')
```
...
---
## Reactomics beyond our lab

- [74+ papers](https://yufree.github.io/reactomics/) using or extending PMD reactomics (2020–2026), curated monthly

- Environmental transformation: FT-ICR MS + PMD networks for DOM in wastewater treatment, composting, photolysis and disinfection byproducts

- PFAS transformation products: high-throughput pathway profiling (UV chain shortening vs plasma oxygen-rich chain lengthening)

- Epidemiology: quantitative PMD (qPMD) on neonatal dried blood spots associated with pediatric acute lymphoblastic leukemia risk

- Data acquisition: PMDDA yields more annotated compounds and molecular networks than CAMERA or RAMClustR

???
- Reactomics is no longer a single-group method — the website collects the literature monthly, spanning environmental chemistry, food chemistry, drug metabolism and exposure epidemiology
- Notable findings: methylation/demethylation are the most conserved transformations across 11 WWTPs; oxygenation precedes chlorination in disinfection byproduct formation
...
### [pmd package](http://yufree.github.io/pmd/)
  ...
  - `getpaired()` / `getchain()` / `getreact()` / `getstd()`
  - GlobalStd algorithm for independent peaks selection
  - Built-in PMD databases: `keggrall`, `hmdb`, `omics`, `sda`, `MaConDa`
  - Reaction-level quantification: static (sum) and dynamic (ratio) modes
  - PMD network analysis and MS/MS annotation algorithm
...
### Real data always enriched in few PMDs — quantized shells frozen by the cofactor set

### Reaction level changes could be quantitative and predictable — reactions are the analytes

???
Here is the take-home message for reactomics: reaction is the "base pairs" for small molecules, reactomics means the omics study at reaction or PMD level, real data is always enriched in few PMDs — discrete shells inherited from the frozen cofactor set, and reaction level changes could be quantitative and predictable, because reactions themselves are the analytes.
