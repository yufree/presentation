---
title: "基于高分辨质谱的反应组学"
subtitle: ""
author: "于淼"
date: "2021-12-15"
output:
  xaringan::moon_reader:
    css: ["default", "my-theme.css", "middlebury"]
    lib_dir: libs
    nature:
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
      ratio: '16:9'
---

```{r setup, include=FALSE}
options(htmltools.dir.version = FALSE)
knitr::opts_chunk$set(fig.align='center',echo = F, cache=T,message=FALSE,warning=FALSE,comment=NA)
```

## 教研经历

- 2006 - 2010 山东大学 环境科学学士 

- 2010 - 2016 中国科学院生态环境研究中心 环境科学博士

  - 多溴联苯醚的植物代谢
  - 导师：江桂斌院士

- 2016 - 2018 加拿大滑铁卢大学化学系 博士后

  - 活体固相微萃取在代谢组学中的应用
  - 导师：Janusz Pawliszyn 院士

- 2018 至今 美国西奈山伊坎医学院暴露组学研究所 博士后

  - 暴露组学数据分析
  - 导师：Lauren Petrick 副教授

---

## 科研产出

- 10篇一作或共同一作文章，39 篇期刊论文，两篇封面，一篇ES&T Letter年度文章，～900 引用，h-index 19

- 发表期刊分布 Communications Chemistry(1) Environ. Sci. Technol.(10) Environ. Pollut.(4) Talanta(4) Environ. Sci. Technol. Lett.(3) Sci. Rep.(3) Anal. Chem.(3) ACS Environ. Au(1) Anal. Chim. Acta(2) Environ. Int. (1) Chemosphare(1) Environ. Sci. Pollut. Res.(1) J. Chromatogr. A(1) Analyst(1) ACS Chem. Neurosci.(1) J. Pharm. Biomed. Anal.(1) Metabolomics(1)

- 3 个开源软件包，CRAN 下载量累计超过 5 万次

- 期刊审稿 J. Hazard. Mater., Anal. Chim. Acta, Talanta, Environ. Pollut., Sci. Total Environ.

- 1 本书章节 *A New Paradigm for Environmental Chemistry and Toxicology: From Concepts to Insights.* Springer Singapore, Singapore, pp. 23–31

- 十几次会议口头报告或邀请报告

---

## 主要研究创新

### 科学问题

- 非目的分析或代谢组学的标准缺失问题

### 基于高分辨质谱的反应组学（reactomics）

- 基于生化反应数据库的数据挖掘

- 基于质量差定性定量样品中化学反应层的变化

- 开发了相关软件与数据库

- 可应用于活体研究、环境非目的分析、未知污染物溯源与疾病诊断

---

# 反应组学

## 传统非目的分析/代谢组学流程

- 样品收集

- 质谱全扫描峰数据采集

- 质谱峰鉴定为物质

- 研究物质间关系

> 样品 -> 质谱峰 -> 物质 -> 物质间关系

- 问题

  - 单样本峰数过万，鉴定耗时
  - 无法覆盖“未知的未知”物质
  
---

## 思路

> 样品 -> 质谱峰 -> ~~物质~~ -> 物质间关系

- 质谱可以直接测量物质关系 (质量差反应组学)

```{r echo=FALSE, out.width= '100%'}
knitr::include_graphics('https://yufree.github.io/presentation/figure/srda.png')
```

---

## 质量差定性

### KEGG 反应数据库

```{r}
example = data.frame(
  stringsAsFactors = FALSE,
                       PMD=c("2.016", "15.995", "79.966", "14.016", "0", "18.011", "162.053", "159.933", "1.032", "42.011"),
  Freq= c(1732L, 1169L, 729L, 594L, 532L, 359L, 365L, 243L, 262L, 237L),
  ExampleReactionClass=c("RC00095", "RC00046 ", "RC00002", "RC00060", "RC00302", "RC00680", "RC00049", "RC02056", "RC00006", "RC00004"),
  ExampleEnzyme = c("1.3.1.84", "1.3.99.18", "3.6.1.3", "1.5.3.1", "5.1.1.3", "3.5.2.5", "3.2.1.23", "4.2.3.42", "1.4.1.2", "2.3.1.54"))
knitr::kable(example,align = 'c',format = 'markdown')
```

---

## 质量差定量

### 静态 v.s. 动态

- 静态质量差: 质量对响应比在样本中相对稳定
- 动态质量差: 质量对响应比在样本中持续变化
- [A,B], [C,D] [E,F] 有相同质量差:

```{r}
GroupI <- c(100,50,'2:1',100,50,'2:1', 30,40, '3:4')
GroupII <- c(1000,500,'2:1',10,95,'2:19',120,160,'3:4' )
df <- rbind.data.frame(GroupI,GroupII)
colnames(df) <- c('A','B','Ins ratio','C','D','Ins ratio','E','F','Ins ratio')
knitr::kable(df,align = 'c',format = 'markdown')
```

- [A,B] 与 [E,F] 属于静态质量差可用来定量, rsd% 阈值 30%
- [C,D] 可用来研究反应动态过程

---

## 质谱峰质量差来源

.pull-left[- 同位素峰 
  
  - $[M]^+$ $[M+1]^+$
  - 1.006 Da

- 离子源反应 

  - $[M+H]^+$  $[M+Na]^+$
  - 21.982 Da
]

.pull-right[
- 同系物

  - 脂肪 $-[CH_2]-$
  - 14.016 Da

- 外源代谢

  - I相反应
  - 15.995 Da
]  

---

## 消除冗余峰

### GlobalStd 算法

```{r echo=FALSE, out.width= '62%'}
knitr::include_graphics('https://yufree.github.io/presentation/figure/GlobalStd.png')
```

.half[
```{r ref2,results = 'asis'}
doi <- c('10.1016/j.aca.2018.10.062')
cat(unlist(rcrossref::cr_cn(dois = doi, format = "text", style = 'apa')))
```
]

---

## 反应组学

### 质量差相当于小分子的碱基对

### 有机物质量差存在富集现象

### 质量差可定性定量指示反应层变化

### GlobalStd 算法利用质量差去除冗余峰

---

## 应用：污染物代谢产物筛查

### 外源代谢

  - I相反应
    - 氧化 (R-H => R-OH, pmd 15.995 Da)
    - 还原 (R-C=O => R-C-OH, pmd 2.016 Da)
    
  - II相反应
    - 甲基化 (R-OH => R-O-C,pmd 14.016 Da)
    - 硫酸化 (R-OH => R-SO4, pmd 46.976 Da)
    - 乙酰化 (R-OH => R-O-COCH3, pmd 42.011 Da)
    - 葡萄糖醛酸化 (R-NH2 => R-NH-C6H9O7, pmd 192.027 Da)
    - 糖基化 (R-OH => R-O-C6H11O5, pmd 162.053 Da)

---

## 四溴双酚A (TBBPA) 植物代谢

- 质量亏损技术筛选溴代污染物 

- 合成标准验证

```{r echo=FALSE,out.width = "600px"}
knitr::include_graphics('https://yufree.github.io/presentation/figure/TBBPAmet.jpeg')
```

.half[
```{r ref3,results = 'asis'}
doi <- c('10.1021/acs.est.9b02122')
cat(unlist(rcrossref::cr_cn(dois = doi, format = "text", style = 'apa')))
```
]

---

## 四溴双酚A (TBBPA) 植物代谢

- 质量差网络分析

```{r echo=FALSE,out.width = "450px"}
knitr::include_graphics("https://yufree.github.io/presentation/figure/TBBPA.png")
```


.half[
```{r ref4,results = 'asis'}
doi <- c('10.1038/s42004-020-00403-z')
cat(unlist(rcrossref::cr_cn(dois = doi, format = "text", style = 'apa')))
```
]

---

## 内源产物 vs 外源产物

- T3DB 数据库 （频率前十的质量差）

```{r echo=FALSE,out.width = "700px"}
knitr::include_graphics("https://raw.githubusercontent.com/yufree/presentation/gh-pages/figure/t3dbnew.png")
```

.half[
```{r results = 'asis'}
doi <- c('10.1038/s42004-020-00403-z')
cat(unlist(rcrossref::cr_cn(dois = doi, format = "text", style = 'apa')))
```
]
---

## KEGG 反应网

- 平均网络距离: Glucose(9.74), 5-Cholestene(6.55), Caffeine(3.31), Bromophenol(1.8), TBBPA(3.88 for pumpkin)

```{r echo=FALSE}
knitr::include_graphics("https://raw.githubusercontent.com/yufree/presentation/gh-pages/figure/keggnew.png")
```

.half[
```{r results = 'asis'}
doi <- c('10.1038/s42004-020-00403-z')
cat(unlist(rcrossref::cr_cn(dois = doi, format = "text", style = 'apa')))
```
]
---

## 肺癌数据集

- MTBLS28 1005 人尿样

- PMD 2.02 Da 可用来区别是否得病

```{r echo=FALSE,out.width="450px"}
knitr::include_graphics('https://yufree.github.io/presentation/figure/mtbls28.png')
```

.half[
```{r results = 'asis'}
doi <- c('10.1038/s42004-020-00403-z')
cat(unlist(rcrossref::cr_cn(dois = doi, format = "text", style = 'apa')))
```
]
---

## 反应组学工具开发

### [pmd 包](http://yufree.github.io/pmd/)
  
  - GlobalStd 算法
  - KEGG/HMDB pmd-reaction 数据库 
  - pmd 反应定量分析
  - pmd 网络分析

### [rmwf 包](https://github.com/yufree/rmwf)

  - NIST 1950 数据 
  - Rmarkdown 模版
  - 从原始数据到结果报告
  - 属于 `xcmsrocker` docker 镜像

---

## 未来研究思路

- 完善反应组学的理论基础

- 活体固相微萃取技术在环境研究中的应用

- 反应组学在环境暴露组学中的应用

- 人群开源数据库的挖掘

---

## 发展规划

### 1-2年：理论研究与数据挖掘为主，搭建实验室平台

### 3-5年：实验室研究为主，面向具体环境问题的研究

### 6-10年：组建暴露组学团队，追踪人群队列，面向具体环境与疾病

---

class: center, middle

# 欢迎各位老师批评指正!

